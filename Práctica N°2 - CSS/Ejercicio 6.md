...
Para reemplazar la hoja de estilo interna por la externa sin modificar el CSS, se deben hacer los siguientes cambios en el HTML:

- Eliminar completamente el bloque `<style>`
- Agregar el link a la hoja externa
- Adaptar ids y clases para que coincidan con estilo2.css:
  - `id="titulo"` → `id="encabezado"`
  - `class="navBar"` → eliminado (ya está incluido en `#menu` en CSS externo)
  - `<ul>` → `<ul class="vineta">`
  - `#pie` → agregar `class="estilopie"`
